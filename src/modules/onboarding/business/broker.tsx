...
import {
  getQuestionaireToLocalStorage,
  setQuestionaireToLocalStorage,
...
const BrokerModal = ({
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    usdot_number: brokerData?.usdot_number ?? savedValues['broker']?.usdot_number ?? '',
    docket_number: brokerData?.docket_number ?? savedValues['broker']?.docket_number ?? '',
    services: brokerData?.services ?? savedValues['broker']?.services ?? [],
    insurance: brokerData?.insurance ?? savedValues['broker']?.insurance ?? [],
    surety_bond: brokerData?.surety_bond ?? savedValues['broker']?.surety_bond ?? []
  });
  ...
  useEffect(() => {
    if (brokerData) {
      const updatedFormData = {
        application_type: 'broker',
        usdot_number: brokerData.usdot_number,
        docket_number: brokerData.docket_number,
        services: brokerData.services || [],
        insurance: brokerData.insurance || [],
        surety_bond: brokerData.surety_bond || []
      };
  ...
  const handleInsuranceAdd = (insurance: Insurance) => {
    const updatedFormData = {
      ...formData,
      insurance: [...formData.insurance, insurance],
    };
    setFormData(updatedFormData);
    setQuestionaireToLocalStorage({ broker: updatedFormData });
    updateQuestionsInfo(updatedFormData, setQuestions, 'broker');
  ...
  const handlesurety_bondAdd = (surety_bond: surety_bond) => {
    const updatedFormData = {
      ...formData,
      surety_bond: [...formData.surety_bond, surety_bond],
    };
    setFormData(updatedFormData);
    setQuestionaireToLocalStorage({ broker: updatedFormData });
    updateQuestionsInfo(updatedFormData, setQuestions, 'broker');
  ...
  const handleSubmit = async () => {
    try {
      const applicationData = {
        ...
        surety_bond: JSON.stringify(formData.surety_bond),
      };

      const application = applicationId
        ? await updateBusinessApplication(applicationData, applicationId)
        : await saveBusinessApplication(applicationData);
  ...
  const serviceOptions = [
    ...
    {
      value: 'Refrigerated Trucks (Reefers)',
      label: 'Refrigerated Trucks (Reefers)',
    },
    ...
    <Modal isOpen={isOpen} onClose={onClose} size="6xl">
      ...
      <ModalContent>
        ...
        <ModalBody>
          ...
          <Box mt={8}>
            <Heading mb={8} color={'Primary.Navy'} fontSize={'xl'}>
              Insurance
            </Heading>
            <TableContainer
              sx={{
        ...
        <ModalFooter>
          <Flex gap={4} maxW={'50%'} margin={'0 auto'} mt={8}>
            <ButtonTheme
              btnText="Back"
              chakraProps={{ w: '100%', onClick: onClose }}
            />
            <ButtonTheme
              btnText="Submit"
              primary
              chakraProps={{ w: '100%', onClick: handleSubmit }}
            />
          </Flex>
        </ModalFooter>
      </ModalContent>
      <AddInsuranceModal
        isOpen={isInsuranceOpen}
        onClose={onInsuranceClose}
        onAdd={handleInsuranceAdd}
      />
      <AddSuretyModal
        isOpen={isSuretyOpen}
        onClose={onSuretyClose}
        onAdd={handlesurety_bondAdd}
      />
    </Modal>
  );
};
