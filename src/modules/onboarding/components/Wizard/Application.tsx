...
const Application = () => {
    ...
    const [questions, setQuestions] = useState(
        {
            broker: { filled: calculateQuestions(savedValues['broker'])?.filledQuestions ?? 0, total: calculateQuestions(savedValues['broker'])?.totalQuestions ?? 5 },
            carrier: { filled: calculateQuestions(savedValues['carrier'])?.filledQuestions ?? 0, total: calculateQuestions(savedValues['carrier'])?.totalQuestions ?? 5 },
            lender: { filled: calculateQuestions(savedValues['lender'])?.filledQuestions ?? 0, total: calculateQuestions(savedValues['lender'])?.totalQuestions ?? 1 }
        }
    );
