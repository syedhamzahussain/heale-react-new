...
const ConnectAccount = () => {
    ...
    const fetchLinkToken = async () => {
      try {
        const response = await fetch(
          process.env.REACT_APP_API_URL + '/api/v2/plaid/create-link-token',
          {
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer 123`, // Assuming you have a way to get the current user's auth token
            },
          }
        );
  ...
  const { open, ready } = usePlaidLink({
    ...
    onSuccess: (publicToken, metadata) => {
      ...
      fetch(process.env.REACT_APP_API_URL + '/api/v2/plaid/exchange-token', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer 123`, // Assuming you have a way to get the current user's auth token
        ...
        .then((data) => {
          ...
          // Proceed with your application logic, e.g., moving to the next step
          nextStep();
        })
